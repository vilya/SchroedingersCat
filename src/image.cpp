...
Image::Image(int type, int bytesPerPixel, int width, int height) :
  _type(type),
  _texId(0),
...
void Image::uploadTexture(unsigned int texId)
{
  GLenum targetType;
  switch (_type) {
    case GL_BGR:
...
void Image::uploadTextureAs(int targetType, unsigned int texId)
{
  if (texId == 0)
...
void Image::downsampleInPlace(unsigned int downsampleX, unsigned int downsampleY)
{
  Image* tmp = downsample(this, downsampleX, downsampleY);
...
unsigned char* Image::takePixels()
{
...
void Image::loadJPG(FILE* file) throw(ImageException)
{
...
void Image::loadPNG(FILE* file) throw(ImageException)
{
...
void Image::loadTIFF(const char* filename) throw(ImageException)
{
...
{
  Image* result = new Image(src->getType(), src->getBytesPerPixel(),
